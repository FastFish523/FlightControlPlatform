...
% Last Modified by GUIDE v2.5 24-Dec-2015 14:02:17
...
t_tolorance=str2num(get(handles.edit_yanshi,'string'));
    ...
    for ct=1:cnt
        for kwy=0.001:0.001:100
            [Gm,Pm,Wcg,Wcp]=margin(-1*kwy*tf_wy(ct));
            t=Pm/Wcp/57.3;
            if t<t_tolorance*2
                break;
            end
        ...
        kwy1(ct)=kwy;
        [p,z]=pzmap(feedback(tf_wy(ct),kwy,1));
        rea=real(min(p));
        ima=imag(min(p));
        zt(ct)=-sign(rea)*cos(atan(ima/rea));
    end
    close(h);
    set(handles.edit_kwz,'string',num2str(kwy1));
...
h = waitbar(0.5,'Please wait...');
if flag==1%×ËÌ¬¿ØÖÆ
    for ct=1:max(size(tf_wy_bihuan))
        wcp=wcp1(ct);
    GA_ph_cu;
    kpp(ct)=kp;
    kii(ct)=ki;
...
jifen=tf([1],[1 0]);
h = waitbar(0.5,'Please wait...');
kwy=str2num(get(handles.edit_kwz,'string'));
cnt=max(size(kwy));
for ct=1:cnt
tf_wy_bihuan(ct)=feedback(tf_wy(ct),kwy(ct),1);
end
save('tf_wy_bihuan.mat','tf_wy_bihuan');
flag=get(handles.popupmenu_choose_loop,'Value');

if flag==1%×ËÌ¬¿ØÖÆ
    close(h);
    for ct=1:cnt
    tf_ph_open(ct)=tf_wy_bihuan(ct)*jifen;
    axes(handles.axes1);
    hold on
    margin(-tf_ph_open(ct));
    aa(ct)=1;
    end
    assignin('base','tf_ph_open',tf_ph_open);
end
if flag==2
    close(h);
    for ct=1:cnt
    tf_nz_open(ct)=tf_wy_bihuan(ct)*tf([V(ct)],[t1_h(ct) 1]);
    axes(handles.axes1);
    hold on
    margin(-tf_nz_open(ct));
    aa(ct)=1;
    end
    assignin('base','tf_nz_open',tf_nz_open);
end
if flag==3
    close(h);
    for ct=1:cnt
...
if flag==1%×ËÌ¬¿ØÖÆ
    for ct=1:max(size(tf_wy_bihuan))
    tf_ph_open=gc(ct)*tf_wy_bihuan(ct)*jifen;
    tf_ph_close=feedback(tf_ph_open,1,-1);
    axes(handles.axes1)
    hold on
    margin(tf_ph_close);
    end
...
if flag==2%¹ýÔØ¿ØÖÆ
 for ct=1:max(size(tf_wy_bihuan))
    tf_nz_open=tf_wy_bihuan(ct)*V(ct)*tf([1],[t1_h(ct) 1]);
    tf_nz_open=gc(ct)*tf_nz_open;
    tf_nz_close=feedback(tf_nz_open,1,-1);
    axes(handles.axes1)
    hold on
    margin(tf_nz_close);
    end
...
for ct=1:cnt
    if flag==3
        tf_wy_bihuan(ct)=feedback(delay*tf_wy(ct),tf([kwy(ct)*t1_h(ct) kwy(ct)+ka(ct)*V(ct)],[t1_h(ct) 1]),1);
    else
        tf_wy_bihuan(ct)=feedback(delay*tf_wy(ct),kwy(ct),1);
    end
end
...
pid=fopen('./cnotrol_para/ph_contorl.txt','w');
...
t_tolorance=str2num(get(handles.edit_yanshi1,'string'));
    ...
    for ct=1:cnt
        for ka=0.001:0.001:100
            [Gm,Pm,Wcg,Wcp]=margin(-1*ka*tf_nz_open(ct));
            t=Pm/Wcp/57.3;
            if t<t_tolorance*2
                break;
            end
        ...
        ka1(ct)=ka;
        [p,z]=pzmap(feedback(tf_wy(ct),ka,1));
        rea=real(p(2));
        ima=imag(p(2));
        zt(ct)=-sign(rea)*cos(atan(ima/rea));
    end
...
for ct=1:cnt
tf_wy_bihuan(ct)=feedback(tf_wy(ct),tf([kwy(ct)*t1_h(ct) kwy(ct)+ka(ct)*V(ct)],[t1_h(ct) 1]),1);
tf_nz_open(ct)=tf_wy_bihuan(ct)*tf([V(ct)],[t1_h(ct) 1]);
end
axes(handles.axes1);
        cla
