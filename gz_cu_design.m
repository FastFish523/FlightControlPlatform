...
% Last Modified by GUIDE v2.5 26-Nov-2015 15:51:57
...
t_tolorance=str2num(get(handles.edit_yanshi,'string'));
    ...
    for ct=1:cnt
        for kwx=0.001:0.001:100
            [Gm,Pm,Wcg,Wcp]=margin(-1*kwx*tf_wx(ct));
            t=Pm/Wcp/57.3;
            if t<t_tolorance*2
                break;
            end
        ...
        kwx1(ct)=kwx;
        [p,z]=pzmap(feedback(tf_wx(ct),kwx,1));
        rea=real(p(1));
        ima=imag(p(1));
        zt(ct)=-sign(rea)*cos(atan(ima/rea));
    end
    close(h);
    set(handles.edit_kwz,'string',num2str(kwx1));
...
for ct=1:max(size(tf_wx_bihuan))
    wcp=wcp1(ct);
    GA_gz_cu;
    kpp(ct)=kp;
    kii(ct)=ki;
...
jifen=tf([1],[1 0]);
h = waitbar(0.5,'Please wait...');
kwx=str2num(get(handles.edit_kwz,'string'));
cnt=max(size(kwx));
for ct=1:cnt
tf_wx_bihuan(ct)=feedback(tf_wx(ct),kwx(ct),1);
end
save('tf_wx_bihuan.mat','tf_wx_bihuan');
flag=get(handles.popupmenu_choose_loop,'Value');

close(h);
for ct=1:cnt
    tf_gz_open(ct)=tf_wx_bihuan(ct)*jifen;
    axes(handles.axes1);
    hold on
    margin(-tf_gz_open(ct));
    aa(ct)=6;
...
assignin('base','tf_gz_open',tf_gz_open);
...
for ct=1:max(size(tf_wx_bihuan))
    tf_gz_open=gc(ct)*tf_wx_bihuan(ct)*jifen;
    tf_gz_close=feedback(tf_gz_open,1,-1);
    axes(handles.axes1)
    hold on
    margin(tf_gz_close);
end
...
cnt=max(size(kwx));
for ct=1:cnt
tf_wx_bihuan(ct)=feedback(delay*tf_wx(ct),kwx(ct),1);
end
...
pid=fopen('./cnotrol_para/gz_contorl.txt','w');
