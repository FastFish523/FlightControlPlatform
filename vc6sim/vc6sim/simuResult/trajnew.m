...
lateral=load('lateralchannel.dat');
else
...
% 
% figure('name','静稳定力矩系数')
% subplot(3,1,1);plot(lateralchannel(:,1),lateralchannel(:,21));grid on; legend('my_beta');ylabel('航向静稳定力矩');grid on;
% subplot(3,1,2);plot(lateralchannel(:,1),lateralchannel(:,25));grid on; legend('mx_beta');ylabel('横向静稳定力矩');grid on;
% subplot(3,1,3);plot(longchannel(:,1),longchannel(:,17));grid on; legend('my_beta');ylabel('纵向静稳定力矩');grid on;
% grid on; xlabel('t(s)');

% figure('name','升阻比-动压')
% subplot(2,1,1);plot(AeroData(:,1),AeroData(:,8));grid on;legend('升阻比');grid on;
% subplot(2,1,2);plot(AeroData(:,1),AeroData(:,9));grid on;legend('动压');grid on; xlabel('t(s)');  ylabel('Q(Pa)');
% 
figure('name','配平比')
...
% % %  
% % % % figure('name','弹体频率 ')
% % % % subplot(2,1,1); plot(body_f(:,1),body_f(:,2)); grid on; ylabel('p(Hz)'); title('横向自振频率');
% % % % subplot(2,1,2); plot(body_f(:,1),body_f(:,3)); grid on; xlabel('t(s)');ylabel('p(Hz)'); title('纵向固有频率');
% % % figure('name','舵偏总量')
...
figure('name','wc')
subplot(2,1,1);plot(longchannel(:,1),sqrt(abs(longchannel(:,10)))/2/3.14,'r');
subplot(2,1,2);plot(lateralchannel(:,1),sqrt(abs(lateralchannel(:,11)))/2/3.14,'r'); 
% 

% figure('name','wc')
% subplot(2,1,1);plot(longchannel(:,1),longchannel(:,10),'r');
% subplot(2,1,2);plot(lateralchannel(:,1),lateralchannel(:,11),'r'); 
