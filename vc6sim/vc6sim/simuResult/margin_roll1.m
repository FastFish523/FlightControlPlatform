...
subplot(3,1,3);  plot(data(:,1),data(:,6)/2/pi,data(:,1),data(:,7)/2/pi,'r');grid;legend('wc(Hz)','wb(Hz)');xlabel('t(s)');
...
subplot(3,1,3);  plot(roll(:,1),roll(:,7));grid;xlabel('Kwz');xlabel('t(s)');
