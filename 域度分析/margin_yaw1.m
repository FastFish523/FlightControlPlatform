...
for jj=1:1:integer/NN
    ...
    if(b24 < 0)
        ...
        body=Grud*tf([ b25  b25*b34-b35*b24 ],[1  -b22+b34 -b24-b22*b34]);
    end
    ...
    [Gm,Pm,wcg,wcp]=margin(-d2r*body*Kwy);
    delay_t(jj) =  Pm/(wcp*57.3)*1000;  %允许延迟时间

    body_kwy= feedback(-AIY*d2r*body,Kwy);
...
% subplot(2,1,2);  plot(lateralchannel(:,1),lateralchannel(:,2),lateralchannel(:,1),lateralchannel(:,2).*yaw(:,11),'r');grid; legend('b25','b25*mean(Q)/Q'); xlabel('t(s)');
