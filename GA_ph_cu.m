...
if flag==1%姿态控制
tf_ph_open(ct)=gc*tf_wy_bihuan(ct)*jifen;
[Gm,Pm,Wcg,Wcp]=margin(tf_ph_open(ct));
end
if flag==2%过载控制
tf_nz_open(ct)=tf_wy_bihuan(ct)*V(ct)*tf([1],[t1_h(ct) 1]);
tf_nz_open(ct)=gc*tf_nz_open(ct);
[Gm,Pm,Wcg,Wcp]=margin(tf_nz_open(ct));
end
if flag==3%过载控制
tf_nz_open(ct)=tf_wy_bihuan(ct)*V(ct)*tf([1],[t1_h(ct) 1]);
tf_nz_open(ct)=gc*tf_nz_open(ct);
