...
% Last Modified by GUIDE v2.5 05-Nov-2015 15:55:27
...
if(zuni_flag==0)
    ...
    for kwx=0.001:0.001:10
    [Gm,Pm,Wcg,Wcp]=margin(-1*kwx*tf_wx);
    t=Pm/Wcp/57.3;
    if t<t_tolorance*2
        break;
    end
    ...
    close(h);
    set(handles.edit_kwz,'string',num2str(kwx));
    set(handles.t1,'string',num2str(kwx));
    [p,z]=pzmap(feedback(tf_wx,kwx,1));
    rea=real(p(1));
    ima=imag(p(1));
...
tf_wx_bihuan=feedback(tf_wx,kwx,1);
save('tf_wx_bihuan.mat','tf_wx_bihuan');
flag=get(handles.popupmenu_choose_loop,'Value');


close(h);
tf_gz_open=tf_wx_bihuan*jifen;
margin(-tf_gz_open);
...
add=(posi-0.5)*5;
kwx_fix=str2num(get(handles.t1,'string'));
kwx=kwx_fix+add;
set(handles.edit_kwz,'string',num2str(kwx));
[p,z]=pzmap(feedback(tf_wx,kwx,1));
rea=real(p(1));
...
delay=tf(num,den);
kwx=str2num(get(handles.edit_kwz,'string'));
tf_wx_bihuan=feedback(delay*tf_wx,kwx,1);
jifen=tf([1],[1 0]);
