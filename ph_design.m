...
% Last Modified by GUIDE v2.5 24-Dec-2015 13:49:27
...
if(zuni_flag==0)
    ...
    [Gm,Pm,Wcg,Wcp]=margin(-1*kwy*tf_wy);
    t=Pm/Wcp/57.3;
    if t<t_tolorance*2
        break;
    end
    ...
    close(h);
    set(handles.edit_kwz,'string',num2str(kwy));
    set(handles.t1,'string',num2str(kwy));
    [p,z]=pzmap(feedback(tf_wy,kwy,1));
    rea=real(min(p));
    ima=imag(min(p));
...
if flag==2
    close(h);
    tf_nz_open=tf_wy_bihuan*tf([v],[t1_h 1]);
    margin(-tf_nz_open);
...
if flag==1%姿态控制
    ...
    tf_ph_open=gc*tf_wy_bihuan*jifen;
...
h = waitbar(0.5,'Please wait...');
    ...
    ki=str2num(get(handles.KI,'string'));
    gc=ki*tf([kp/ki 1],[1 0]);
    tf_nz_open=tf_wy_bihuan*v*tf([1],[t1_h 1]);
    tf_nz_open=gc*tf_nz_open;
...
add=(posi-0.5)*5;
kwy_fix=str2num(get(handles.t1,'string'));
kwy=kwy_fix+add;
set(handles.edit_kwz,'string',num2str(kwy));
[p,z]=pzmap(feedback(tf_wy,kwy,1));
rea=real(p(1));
...
if flag==2%过载控制
    tf_nz_open=tf_wy_bihuan*v*tf([1],[t1 1]);
    gc=-1*ki*tf([kp/ki 1],[1 0]);
    tf_nz_open=gc*tf_nz_open;
    margin(tf_nz_open);
end
...
if flag==3
tf_wy_bihuan=feedback(delay*tf_wy,tf([kwy*t1_h kwy+ka*v],[t1_h 1]),1);
else
tf_wy_bihuan=feedback(delay*tf_wy,kwy,1);
end
jifen=tf([1],[1 0]);
if flag==1%姿态控制
    h = waitbar(0.5,'Please wait...');
...
for ka=0.001:0.001:100
    [Gm,Pm,Wcg,Wcp]=margin(-1*ka*tf_nz_open);
    t=Pm/Wcp/57.3;
    if t<t_tolorance*2
        break;
    end
...
set(handles.edit_ka,'string',num2str(ka));
 [p,z]=pzmap(feedback(tf_nz_open,ka,1));
    rea=real(p(2));
    ima=imag(p(2));
    zt=-sign(rea)*cos(atan(ima/rea));
    set(handles.edit_zuni,'string',num2str(zt));
hold off
margin(-1*ka*tf_nz_open);
...
kwy=str2num(get(handles.edit_kwz,'string'));
ka=str2num(get(handles.edit_ka,'string'));
tf_wy_bihuan=feedback(tf_wy,tf([kwy*t1_h kwy+ka*v],[t1_h 1]),1);
tf_nz_open=tf_wy_bihuan*tf([v/9.8],[t1_h 1]);
margin(-tf_nz_open);
assignin('base','tf_nz_open',tf_nz_open);
save('tf_wy_bihuan.mat','tf_wy_bihuan');
